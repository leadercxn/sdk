...
#endasm
#elif defined(_IAR_)
  __asm("ld a, S:SamplingShifter    \n" //s: to use a zero page addressing mode
  // 3 cycles loop if jump executed; 2 cycles loop if not.
        "ShiftLoopVih:              \n"
        "dec a                      \n"        // 1 cycle
        "jrne ShiftLoopVih          \n"
...
#endasm
#elif defined(_IAR_)
  __asm("ld a, S:AcquisitionBitMask    \n" //s: to use a zero page addressing mode
        "ldw x, S:sTouchIO             \n" // Input data register ...  //s: to use a zero page addressing mode
        "incw x                        \n"
  // Loop = 1 + 1 + 2 + 2 + 2 cycles = 8 cycles
        "WaitForVil:                   \n"
// To be sure that the loop last 8 cycles the first instruction must be a 1-byte instruction
...
// If this not the case, the code must be aligned.	
        "bcp a, (x)                    \n"  // 1 cycles
        "jreq EndWaitForVil            \n"
        "ldw y, TIMACQ_CNTR            \n" // 2 cycles; hw counter also used for timeout ...
        "cpw y, #0x0E00                \n"    // 2 cycles; Timeout compare
        "jrult WaitForVil              \n"
        "EndWaitForVil:                \n"
  );
#else //_RAISONANCE_
