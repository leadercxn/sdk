...
uint32_t app_fifo_puts(app_fifo_t * p_fifo, uint8_t* p_data, uint16_t len)
{
    uint32_t err_code;
    for(uint16_t i = 0; i < len; i++)
    {
        err_code = app_fifo_put(p_fifo, p_data[i]);
...
uint32_t app_fifo_gets(app_fifo_t * p_fifo, uint8_t *p_buff, uint16_t len)
{
    uint32_t err_code;
    uint16_t counter = 0;
    for(uint16_t i = 0; i < len; i++)
    {
        err_code = app_fifo_get(p_fifo, &p_buff[i]);
        if(NRF_SUCCESS != err_code)
        {
            return counter;
        }
