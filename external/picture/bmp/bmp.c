		...
		{				 
			while (count < readlen)  //读取一簇1024扇区 (SectorsPerClust 每簇扇区数)
			{
				...
				{		 
					...
					if (is_element_ok(realx, realy, 0))			//此处不改变picinfo.staticx,y的值	 
					{
						yok = 1;
					}
					else 
...
//小尺寸的bmp解码(小于240*320),解码filename这个BMP文件		
//filename:包含路径的文件名
...
{				
	...
	little_memset((uint8_t*)&hbmp,0,sizeof(hbmp));	//置零空申请到的内存.
	...
	if (mode == 0 || res == 0x04)			//模式0,或者尝试打开失败,则创建新文件		   
	{
		res = f_open(f_bmp, (const TCHAR*)filename, FA_WRITE|FA_CREATE_NEW);
	}
	if ((hbmp.bmiHeader.biWidth * 2) % 4)		//水平像素(字节)不为4的倍数, 实际要写入的宽度像素,必须为4的倍数.	
	{
