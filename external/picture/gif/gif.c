...
{
 	little_memset((uint8_t *)gif->lzw, 0, sizeof(LZW_INFO));
...
uint8_t gif_readextension(FIL *gfile, gif89a* gif, int *pTransIndex, uint8_t *pDisposal)
{
	...
	switch (temp)
	{
		...
		case GIF_GRAPHICCTL:								//图形控制扩展块
			if (gif_getdatablock(gfile, buf, 4) != 4)		//图形控制扩展块的长度必须为4 
			{
				return 1;	
			}
...
int gif_getnextbyte(FIL *gfile, gif89a* gif) 
{
	...
	while ((Code = gif_getnextcode(gfile,gif)) >= 0)
	{
		if (Code == gif->lzw->ClearCode)
		{
			...
			if (gif->lzw->ClearCode >= (1 << MAX_NUM_LWZ_BITS))		//Error 
			{
				return -1;
			}

			//Clear the tables 
			little_memset((uint8_t*)gif->lzw->aCode, 0, sizeof(gif->lzw->aCode));
		...
		if (Code == gif->lzw->EndCode)		//End code
		{
			return -2;
		}
...
uint8_t gif_dispimage(FIL *gfile, gif89a* gif, uint16_t x0, uint16_t y0, int Transparency, uint8_t Disposal) 
{
	...
	for (YCnt = 0, YPos = y0, Pass = 0; YCnt < Height; YCnt++)
	{
		...
		for (XPos = x0; XPos <= XEnd; XPos++)
		{
			...
			if ((Index < 0) || (Index >= gif->numcolors))	//IfIndex out of legal range stop decompressing
			{
				return 1;//Error
			}
...
void gif_clear2bkcolor(uint16_t x, uint16_t y, gif89a* gif, ImageScreenDescriptor pimge)
{
	...
	if (pimge.width == 0 || pimge.height == 0)					// 直接不用清除了,原来没有图像!!
	{
		return ;
	}
	...
	if (gif->gifISD.yoff > pimge.yoff)
	{
		...
		if (x0 < x1 && y0 < y1 && x1 < 320 && y1 < 320)			// 设定xy,的范围不能太大.
		{
		...
		{		 
			case GIF_INTRO_IMAGE://图像描述
				...
				{							  
					...
					if (gif_readcolortbl(gfile, gif, numcolors))			//读错误
					{
						return 1;	
					}
				}
				...
				while(1)
				{
					...
					if (f_lseek(gfile, readed + temp))					//继续向后偏移
					{
						break;	 
					}
				}	
	...
	if (res == 0)								//OK
	{
		...
		if (res == 0)							//打开文件ok
		{
			...
			{	 
				...
				if (mygif89a->gifISD.flag & 0x80)					//恢复全局颜色表
				{
				...
				{
					g_tftlcd_lvgl_obj->tftlcd_cfg.delay_ms(10);
				}
