...
// uint8_t mem2base[MEM2_MAX_SIZE] __attribute__ ((aligned(32)));
...
{
	little_mem_init,				//内存初始化
	little_mem_perused,			    //内存使用率
	mem1base,			            //内存池
	mem1mapbase,	                //内存管理状态表
	0,   		 				    //内存管理未就绪
};
...
{  
    little_memset(mallco_dev.memmap[memx], 0, memtblsize[memx] * 2);    //内存状态表数据清零  
	little_memset(mallco_dev.membase[memx], 0, memsize[memx]);	        //内存池所有数据清零  
...
{  
    ...
    little_mem_free(memx, offset);	        //释放内存      
}  
...
{  
    ...
    {  									   
	    little_memcpy((void*)((uint32_t)mallco_dev.membase[memx]+offset), ptr, size);	//拷贝旧内存内容到新内存   
        little_free(memx, ptr);  											  		    //释放旧内存
