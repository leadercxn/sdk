...
int hk_exit4_15_init(exit_cfg_t *p_exit_cfg)
{
    ...
    hk_exit_cfg *p_hk_exit_cf = (hk_exit_cfg *)p_exit_cfg->p_exit_cfg;

    EXTI_InitTypeDef   EXTI_InitStructure;
    GPIO_InitTypeDef   GPIO_InitStructure;
    NVIC_InitTypeDef   NVIC_InitStructure;

    RCC_AHBPeriphClockCmd(p_hk_exit_pin_cfg->exit_pin_clk, ENABLE);
...
//    RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG, ENABLE);
    ...
    EXTI_InitStructure.EXTI_Line = p_hk_exit_cf->exit_line;  
    EXTI_InitStructure.EXTI_Mode = p_hk_exit_cf->exit_mode;
    EXTI_InitStructure.EXTI_Trigger = p_hk_exit_cf->exit_trigger;
...
{
  hk_exit_cfg *p_hk_exit_cf = (hk_exit_cfg *)g_exit4_15_obj.exit_cfg.p_exit_cfg;

  if(EXTI_GetITStatus(p_hk_exit_cf->exit_line) != RESET)
  {
        EXTI_ClearITPendingBit(p_hk_exit_cf->exit_line);
