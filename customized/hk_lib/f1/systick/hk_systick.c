...
void hk_systick_init(systick_cfg_t *p_sys_config)
{
	...
	// 1. 设置delay_us/ms计数基准
	p_sys_config->us_ticks = 9;

	// 2. 获取systick
	p_sys_config->p_systick = &m_sys_tick;

	// 3. 使能嘀嗒定时器
	reload	=	SystemCoreClock / 8000000;
	reload	*=	1000000 / p_sys_config->ticks_per_sec;
	systick_start();
}
...
void hk_delay_us(systick_cfg_t *p_sys_config, uint32_t us)
{
	...
	SysTick->LOAD = (uint32_t)(us * p_sys_config->us_ticks);
...
void hk_delay_ms(systick_cfg_t *p_sys_config, uint32_t ms)
{
	...
	{
		//	volatile uint32_t temp;
		SysTick->LOAD = (uint32_t)(1000 * p_sys_config->us_ticks);
		SysTick->CTRL = 0x00; //停止计数
		SysTick->VAL = 0x00;  //清空计数器
		SysTick->CTRL = 0x01; //开始计数
							//	temp  = SysTick->VAL;
